...
### 5. 专线调度哲学升级：网速优先与同区低延 (Speed-First & Geo-Locality)
- 废除陈旧的“强制美国优先”标签，升级为“网速优先（低延迟与可用性优选）”。
- 顺应 Google 账号风控同区稳定原则：保持 Antigravity 出口与用户日常环境在同一个国家/地区（日本），避免跨洋异地 IP 跳变触发异常登录风控。
- 遭遇 Google 地区风控（User location is not supported）时，自动触发跨节点高速探测并无感热重载至可用专线（如日本/美国高速线路）。
- 启动器拟态徽标升级为 `⚡ 网速优先`、`🌐 同区低延`、`⭐ 记忆好用`，默认专线状态升级为 `高速专线就绪`。
