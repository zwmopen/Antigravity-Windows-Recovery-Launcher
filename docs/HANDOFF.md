...
- `Antigravity-ProxySupervisor.ps1`：监督器 2.8.1 从 Clash Verge 与 Mihomo Party 订阅索引定位仍有效的本地缓存，发现跨来源日本/美国候选，维护失败冷却，配置/启动 17897，并以官方 `agy` 最小真实生成作为隔离门禁和正式 17897 二次门禁；US/JP 共同进入低延迟池，不再按国家硬编码优先；新增脱敏候选数量、来源统计和失败状态事件供中文启动器显示。
...
- 当前源码恢复链版本：1.6.6；监督器 2.8.1、AccountWatcher 0.5.6、启动器 0.9.1。安装目录与源码副本必须用哈希分别核对，不能仅凭文件时间判断已经生效。
