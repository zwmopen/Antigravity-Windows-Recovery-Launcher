...
为 Cockpit 多账号切换后的 Antigravity 自动检查专用代理、清除黑屏实例、恢复启动，集成 CCOCK 选号引擎与无人值守全自动续航守护神，提供全链路脱壳自愈、订阅状态透出与可逆外部简体中文 UI 扩展。单一权威开源代码仓库：`zwmopen/Antigravity-Windows-Recovery-Launcher`。
