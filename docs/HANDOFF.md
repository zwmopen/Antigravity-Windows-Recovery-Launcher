...
- 2026-09-15 本地监督器隔离探测升级：新增 Invoke-IsolatedCandidateProbe，在独立临时端口测延迟并验证模型，按当前 RTT 排序；候选失败不改正式 PID/配置，全池失败不再停止正式监听。仅验证通过后进入接管，接管异常恢复旧配置。已部署脚本和同步 manifest，尚未发布新安装包。
- 验证：Windows PowerShell 5.1 语法、两个失败候选及成功路径的隔离清理模拟测试通过；实机探针地区 400 时生产 PID/配置哈希不变。成功接管与接管失败回退的真实端到端演练尚未完成，不能称全量验收。旧恢复进程曾干扰第一次实测，后续旧轮次已自行结束。
