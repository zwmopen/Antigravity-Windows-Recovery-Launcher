...
- 桌面同时维护 `Antigravity 原版.lnk`，直接指向官方 `Antigravity.exe`；它绕过恢复门禁，仅作为用户需要时的直开入口，不能当作专线/模型已验收。
...
- 2026-09-16 1.6.8 / Watcher 0.6.0 修复：现场 12:08–12:15 证明客户端地区 400 会让旧 Watcher 每分钟左右重启 `17897`，随后客户端收到本地端口拒绝；而监督器的正式 AGY 门禁每次均通过。新增地区 400 30 秒防抖、成功后 120 秒稳定观察、15 分钟熔断和失败周期熔断；真实 Google/OAuth 断流仍按连续 3 次进入网络恢复。代码级回归已通过，实机安装与运行观察待本轮完成。
- 2026-09-16 本地监督器恢复修复：候选临时端口通过后，正式 `17897` 再执行一次 `agy` 真实模型门禁；正式门禁失败会回滚旧配置并继续候选轮询。US/JP 不再按国家硬编码排序，候选池按地区均衡取样后以实际 RTT 和模型门禁决定。
- 2026-09-16 00:37–00:51 实机验证：隔离探测通过但正式 `17897` 失败的候选被正式门禁拦截并回滚；随后 7 条候选均未通过时记录 `no_replacement_production_preserved`，没有把坏节点标为 ready。日常 `7897` 进程 PID 7632 全程未变。
- 2026-09-16 01:01 冷启动验收：候选 `C08400083F1ADCEF` 在正式 `17897` 二次门禁通过；启动器以 `--proxy-server=http://127.0.0.1:17897` 启动 Antigravity，PID 25348、语言服务 PID 16800、代理连接 10 条，中文 Loader 成功，`17897` 当前由私有 Mihomo PID 17196 持有。启动后到 01:03 未发现新的位置 400、`proxyconnect` 拒绝或额度错误。
- 验证边界：原有客户端在 00:44:16 仍对 Trajectory `2bea2ed0-c592-487b-a033-2bab62ae7f10` 返回 Google `FAILED_PRECONDITION 400`，说明“正式 `agy` 通过”不能替代现有客户端新请求验收；本次已通过冷启动重建客户端并完成启动/语言服务/代理验收，用户新发一条请求仍是最终业务确认。当前错误本质是 Google 上游账号资格与出口组合，不是 `7897` 或本地 17897 TCP 拒绝。
