...
- 修复：0.5.0 后台每 20 秒检查 Google 与 OAuth；连续 3 次失败才隔离当前候选 20 分钟，从当前活动订阅选择下一条洛杉矶候选并无窗口恢复。
...
- 修复：0.1.4 在启动阶段通过 `17897` 请求 Cloudflare trace，解析 `loc`，仅允许 `US` 继续启动；其他地区或无法判定时安全停止。
