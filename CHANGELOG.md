...
- **热启动双选胶囊卡片与极简人话对齐（方案 2 + 极简 A 款）**：
  - **交互体验重构**：当 Antigravity 已在后台运行时，双击桌面启动器不再直接静默穿透置顶，而是呼出精致的 480×146 原生 DWM 圆角拟态双选胶囊卡片，动态展示当前在用专线出口国家与通道连接状态。
  - **4字工整对称人话设计**：
    - 主操作：**`🚀 直接打开 (3s)`**，配备 3 秒动态倒计时，无操作自动唤醒代码窗口；支持回车或空格键秒切；
    - 救急操作：**`⚡ 重启修复`**，深色次级按钮，留白舒适无拥挤感；点击即平滑杀旧进程，一键触发重新优选节点并冷启动自愈。
  - **键盘与事件穿透优化**：支持 Esc 或右上角关闭静默退出，卡片任意空白区域支持左键拖动；
  - **并发单实例保障**：单实例 Mutex 检查提前至卡片展示之前，彻底杜绝快速双击导致的重复弹窗；
  - **桌面入口统一**：清理历史散落测试包，桌面维持唯一快捷方式 `Antigravity 启动器.lnk`。
