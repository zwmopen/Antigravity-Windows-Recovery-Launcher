...
- **后台边界保持不变**：仅前台启动链传入 `-PrelaunchClient`，AccountWatcher 的静默后台恢复不会主动弹出客户端窗口。
- **新增回归契约**：覆盖前台开关、启动顺序与脱敏启动事件；已完成实机桌面冷启动、窗口就绪和中文 Loader 验收。

## 1.6.19 修复 Antigravity 2.14 冷启动健康误判 — 2026-09-18

- **修复语言服务发现**：Antigravity 2.14 可能通过 Electron utility 进程启动 `language_server.exe`，监督器不再只查找主进程的直接子进程，同时保留旧版本父子关系兼容。
- **放宽错误的窗口句柄门禁**：以新鲜语言服务、初始化日志、主进程响应和 17897 连接作为启动就绪依据，避免健康窗口因主 Electron 进程没有顶层窗口句柄被误杀。
- **新增回归契约**：覆盖语言服务发现方式和启动健康判定边界。

## 1.6.18 消除25秒硬等待、全流程断点续接极致压缩至6秒极速响应 — 2026-09-18

- **全面消灭人工硬性等待（Latency Compression）**：
  - **消除 10 秒空转盲等**：移除 `execute_auto_resume` 启动阶段冗余的 `time.sleep(10.0)`，因热重启 DevTools 端口一直保活且页面在后台已渲染，直接缩短至 `1.0s` 即可完成 DOM 挂载感知；
  - **沉降等待大幅收敛（5.0s -> 1.2s）**：将输入框沉淀及回车前等待从 `5.0s` 降至 `1.2s`，结合前端已有的 15 次重试轮询（1.5s）实现毫秒级快速确认；
  - **导航与换页过渡优化（2.0s -> 0.8s）**：将 TanStack Router 内部路由切换及返回前台分屏的等待由 `2.0s` / `1.5s` 统一压缩至 `0.8s`；
  - **发送后安全停顿压缩（1.0s -> 0.4s）**：单列或后台任务发送成功后的冷却时间由 `1.0s` 压缩至 `0.4s`；
  - **重试间隔减负（30s -> 3s）**：侧边栏未加载完成时的单次轮询等待由 `30s` 降至 `3s`；
- **全链路实测收益**：
  - 热重启后从发现断点到各窗口轮流补发“继续”，全程耗时由原本积累的 **32+ 秒** 极致缩减至 **~6.5 秒**，提速达 **80%**，彻底杜绝用户感知到的“等了好久才点其他窗口”的漫长卡顿。

## 1.6.17 根治误向主页/空白新对话误发'继续'痛点、前置会话路由对齐与主页物理防护 — 2026-09-18

- **前置真实会话路由对齐（Pre-alignment）**：
  - 攻克热重启后 Electron 窗口重置停留在主页或新建空白会话 (`/` 或 `/c`) 时，脚本盲目直接在当前屏幕打字的严重乌龙；
  - 在执行任何续接输入前，若切号前处于某个真实工作会话 (`/c/<id>`)，强制优先通过 CDP 相对路由瞬切对齐到该目标会话，等待加载就绪后再键入；
- **空白新会话与主页绝对物理免疫门禁**：
  - 在 `prep_pane_js` 注入页面状态与消息深度探针：若当前路由为 `/`、`/c` 且页面没有任何历史对话消息 (`chat-turn` / `message`)，判定为初始空白态，直接触发 `blank_home_page_skip` 立即安全跳过，绝不向新对话或主页空打“继续”。

## 1.6.16 恢复自动发“继续”能力、实装单窗口直接接力与侧边栏智能续接闭环 — 2026-09-18

- **全面恢复自动发“继续”能力并开启启动器配置**：
  - 更新 `launcher-settings.json` 的 `auto_resume_enabled` 为 `true`，同时脚本内部默认 fallback 同步置为 `True`；
  - 针对单窗口主流场景（放弃多分屏），消除 429 报错后前端转圈停转被误判为空闲跳过的痛点，单窗口下始终直接接力当前主会话；
  - 完整保留并加固侧边栏上次未完成会话识别与切换跳转接力链路，续接完成后原路返航复原主会话。

## 1.6.15 彻底关闭自动发“继续”功能、全面回归极简静默无感切号 — 2026-09-17

- **关闭自动发“继续”与跨页面 CDP 接力（返璞归真）**：
  - 将 `launcher-settings.json` 的 `auto_resume_enabled` 设为 `false`，同时将脚本默认 fallback 改为 `False`；
  - 彻底关闭 CDP 自动输入、回车提交以及跨侧边栏会话的 `navigate` / `pushState` 行为，杜绝多窗口、多分屏时的页面乱跳与焦点干扰；
  - 维持核心的 100% 后台无感满血切号、专线保活与语言服务热重启，换号后不扰乱任何当前视图与输入框，由用户掌控各分屏的续接时机。
- **修复底层变量作用域与健壮性兜底**：
  - 修复 `_cdp_execute_auto_resume` 中 `wait_enter_sec` 仅在条件分支内声明导致的 `UnboundLocalError`；
  - 为输入框与发送按钮增加异步轮询等待，避免页面未就绪时空打标。
...
- **用户自愈重连（Repair）强制重启机制修复**：
  - 根治启动器在用户点击“⚡ 网络自愈重连”时未正确传递 `UserRequestedRepair`、导致误入平滑附着（未实际杀死挂死窗口）的 Bug；修复后显式触发 `Stop-ExistingAntigravity`，彻底清除断连挂死进程并重拉健康实例。
- **双阶段汉化注入与全页面目标覆盖（彻底消灭漏译与英文回退）**：
  - `Antigravity-CdpLocalizationLoader.exe` 升级为全 Target 遍历注入（匹配所有 `type: page` 目标），杜绝多窗口/渲染容器漏译；
  - 监督器构建“首帧并发挂载 + 语言服务就绪后 DOM 闭环锁定”双保险时序，彻底解决 Electron 页面冷启动导航导致前期注入被刷新冲掉的问题。
...
- **热启动与交互直觉终极定版**：
  - 左侧主按钮定版为 `打开继续使用 (3s)`，消除进入编辑器的阻力与严肃感；
  - 右侧辅助按钮定版为 `⚡ 网络自愈重连`，强化日常工具属性，彻底消除用户对系统损坏的心理疑虑。
- **标题栏三点菜单（`titlebar-more-actions`）修复与汉化**：
  - 精准收缩屏蔽规则，消除历史误伤三点菜单的 CSS 宽选择器；
  - 将 `role="menuitem"`, `role="menu"`, `role="option"`, `role="tab"` 等全部 WAI-ARIA 菜单角色纳入 UI 控件识别范围；
  - 补齐重命名、固定/取消固定、归档/恢复、远程控制、分屏（向右/向下/替换为新建/从分屏中移除）、复制（对话名称/ID/工作区/项目名）、终端、派生、分享等全量会话菜单项。
- **零闪烁（Zero-FOUT）无感翻译架构固化**：
  - `document_start` 与 CDP `Page.addScriptToEvaluateOnNewDocument` 提前驻留，首帧渲染前完成准备；
  - 微任务周期同步拦截（`isInstantUiNode`），在浏览器 Layout/Paint 绘制前 0ms 完成 DOM 文本与属性覆写，彻底消灭视觉跳字。
- **全界面深层汉化地毯式排查补齐**：
  - AI 深度思考分钟/小时级时间覆写（`深度思考 X 分钟 Y 秒` / `深度思考 X 小时`）；
  - 多厂商模型思考模式通用适配（Claude/GPT/Gemini，如 `Claude Sonnet 4.6（思考模式）`）；
  - 代码变更摘要（`X 个修改的文件`）与审查操作（`审查`、`未提交`、`已使用的技能`）；
  - 面板缩放与无障碍属性（`辅助面板`、`最大化/还原面板`、`概览/审查/终端标签页`、`取消 (Ctrl+D)`、`停止执行`、`滚动到底部`、`折叠/展开排队消息`、`查看用量明细` 等）。
- **交付与测试**：单元测试 100% PASS，`build.ps1` 与 `install.ps1` 编译部署闭环，实机 CDP 热注入验证通过。

## 1.6.9 专属网络纯净保活与多维健康技能联动 — 2026-09-16

- **IPv6 旁路回国风险彻底根除**：监督器候选池过滤所有 IPv6 节点并强制 `ipv6: false`，彻底杜绝本地电信/联通 IPv6 旁路导致 Google 判定为 Mainland China 而触发 `400 Bad Request: User location is not supported`。
- **长连接 SSE 物理防掐断**：在 Clash Verge 中锁定 `auto_close_connection: false`，彻底消灭因后台测速切换节点强杀活跃 TCP 连接导致的反重力“The stream was interrupted”流中断。
- **Clash 界面两层直觉化架构 (v4.0.0)**：重构 `Script.js`，实现 `1️⃣ 业务主控`（AI专属、日常外网、苹果服务）与 `📍 地区机房池`（日本保活、香港低延、新加坡、美国）两层解耦；各主控组选项严格统一，物理过滤订阅公告和假节点。
- **模式 A 独立沙盒持续保活**：反重力独享 `127.0.0.1:17897` 私有 Mihomo 实例，与日常浏览器 `7897` 完全物理隔离。日常在 Clash 界面随心切换节点，反重力专线岿然不动。
- **启动器人机交互与大白话体验升级 (UI/UX 焕新)**：
  - **热启动卡片去沉重化**：左侧按钮优化为 `回到编辑器 (3s)`，右侧辅助按钮优化为轻量化日常工具感 `⚡ 刷新专线 / 换节点`（彻底摒弃令人焦虑的“一键重启修复”字样）；副标题明确标识 `专线稳定畅通 · 点击秒切编辑器，或快速换节点自愈`；
  - **设置面板大白话注解**：针对右上角 ⚙ 设置里的 `auto_resume_enabled`，重构为主选项 `切号或网络重连后，自动恢复会话与任务上下文`，并附带副标解释“账号切换或专线刷新后自动续接未完成任务；关闭则仅静默换号”；
  - **桌面入口唯一权威化 (Point 3)**：桌面彻底收敛为单一官方蓝标入口 `Google Antigravity.lnk`（双击直达智能启动器），原版快捷方式 `Antigravity 原版.lnk` 自动迁移至开始菜单底层防翻车兜底，消除桌面双图标心智负担。
- **联动多维系统健康诊断官 (system-health-diagnostician)**：正式注册全新诊断技能，将启动器看门狗日志、17897/7897 专线质量与 Codex 性能体检、AICode 顶层架构打通，支持用户自然语言“找找问题/检查系统/看下日志”触发，7秒内输出三维体检报告。

## 1.6.8 客户端地区 400 防抖与 Watcher 版本对齐 — 2026-09-16

- **停止地区 400 重启循环**：客户端出现 `User location is not supported` 时先合并 30 秒，再进行一次有界候选轮换；轮换成功后观察 120 秒，短时间内再次出现则打开 15 分钟自动轮换熔断，避免每次客户端请求都重启 `17897` 并制造 `proxyconnect actively refused`。
- **保留真正网络自愈**：Google/OAuth 端口健康连续 3 次失败仍进入网络恢复；地区错误熔断只约束地区错误，不阻断真实断流修复。
- **修复运行时版本漂移**：AccountWatcher 程序集、日志版本和 `WATCHER_VERSION` 统一为 `0.6.0`；启动器程序集统一为 `1.6.8`，避免源码已升级但正在运行的旧 EXE 继续执行旧策略。
- **保留官方桌面入口**：安装/升级时同步维护 `Antigravity 原版.lnk`，不再把用户明确需要的官方直开入口列入旧快捷方式清理名单。
- **现场依据**：12:08–12:15 的日志显示同一客户端连续地区 400，监督器每次 AGY 正式门禁均通过但仍反复切换；这属于客户端/上游资格与守卫触发策略叠加，不是日常 `7897` 的故障。日常 Clash、登录态、会话与官方客户端二进制不改。

## 1.6.7 冷启动模型门禁修复 — 2026-09-16

- **修复冷启动 15 秒假等待**：2.8.3 在 Antigravity 尚未启动时强制等待已有 `language_server.exe`，导致每条基础链路通过的候选都被误判为 `model_transport`；现在由官方 `agy` 自行负责 language server 的启动/复用，真实错误回到 AGY 结果分类。
- **减少重复模型消耗**：临时端口候选阶段只做 Google/OAuth/出口预检；正式候选接管 `17897` 后只执行一次真实 AGY 模型门禁，避免同一候选重复消耗账号额度。
- **修复 watchdog 误回滚**：守护脚本改为比较监督器版本与 golden copy，不再把隔离探针故意使用的局部 `3000ms` 超时识别为旧 bug。
- **回滚边界不变**：正式模型门禁失败仍恢复上一份专用配置；日常 Clash `7897`、登录态、会话和官方客户端二进制不变。

## 1.6.6 本地网络恢复修复 — 2026-09-16

- **正式 17897 二次模型门禁**：候选临时端口通过后，接管正式代理仍必须再次通过 `agy` 真实生成；失败会回滚正式配置并继续候选轮询，不再把一次隔离通过误报为 `ready`。
- **日美同池、低延迟优先**：移除 US 硬编码优先级，US/JP 进入同一测速池；候选池超限时按地区均衡取样，最终以新鲜 RTT 与真实模型门禁共同决定。
- **故障证据收口**：保留正式 17897 监听边界，并记录正式模型门禁开始、通过和失败事件。
