...
$TargetAlias = 'ANTIGRAVITY-US1'
$TargetNodePattern = '美国1'
...
function Read-TargetNode {
    if (-not (Test-Path -LiteralPath $ProfilesIndex)) {
        Stop-WithMessage -Event 'profiles_index_missing'
    }

    $indexText = Get-Content -LiteralPath $ProfilesIndex -Raw
    $currentMatch = [regex]::Match($indexText, '(?m)^current:\s*(?<id>[A-Za-z0-9_-]+)\s*$')
    if (-not $currentMatch.Success) {
        Stop-WithMessage -Event 'current_profile_missing'
    }

    $currentId = $currentMatch.Groups['id'].Value
    $profilePath = Join-Path $ProfilesRoot ($currentId + '.yaml')
    if (-not (Test-Path -LiteralPath $profilePath)) {
        Stop-WithMessage -Event 'current_profile_file_missing'
    }

    $candidateLines = @(Get-Content -LiteralPath $profilePath | Where-Object {
        $_ -match '^\s*-\s*\{.*type:\s*vmess.*\}\s*$' -and $_ -match $TargetNodePattern
    })
    if ($candidateLines.Count -ne 1) {
        Stop-WithMessage -Event 'target_node_not_unique'
    }

    $nodeLine = $candidateLines[0].Trim()
    if ($nodeLine -notmatch '^\-\s*\{.*\}$') {
        Stop-WithMessage -Event 'target_node_format_invalid'
    }

    # Rename only the local YAML key. All transport fields remain from the active profile.
    $nodeLine = $nodeLine -replace "(?i)name:\s*(?:'[^']*'|[^,}]+)", ('name: ' + $TargetAlias)
    if ($nodeLine -notmatch ('name:\s*' + $TargetAlias)) {
        Stop-WithMessage -Event 'target_node_alias_failed'
    }
...
$target = Read-TargetNode
$configState = Write-PrivateConfig -ProfileId $target.ProfileId -NodeLine $target.NodeLine
Test-PrivateConfig
