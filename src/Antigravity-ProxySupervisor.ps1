...
function Invoke-IsolatedCandidateProbe {
    ...
    $ProxyRoot = Join-Path $ProxyRoot ('probe-' + [guid]::NewGuid().ToString('N'))
    $ConfigPath = Join-Path $ProxyRoot 'mihomo-antigravity.yaml'
    $StatePath = Join-Path $ProxyRoot 'state.json'
    $probeProcess = $null
    # Probe errors are not production failure-state transitions.
    function Stop-WithMessage { param([string]$Event) throw $Event }
    try {
        $null = Write-PrivateConfig -ProfileId 'isolated-probe' -Candidate $Candidate
        Test-PrivateConfig
        $probeProcess = Start-Process -FilePath $MihomoPath -ArgumentList @('-d', ('"' + $ProxyRoot + '"'), '-f', ('"' + $ConfigPath + '"')) -WindowStyle Hidden -PassThru
        for ($attempt=0; $attempt -lt 40; $attempt++) {
            if ($probeProcess.HasExited) { throw 'probe_process_exited' }
            if (Test-LocalPort -TestPort $Port) { break }
            Start-Sleep -Milliseconds 100
        }
        if (-not (Test-LocalPort -TestPort $Port)) { throw 'probe_port_timeout' }
        Write-SafeLog -Event 'isolated_probe_started' -Values @{port=$Port; pid=$probeProcess.Id}
        if ($ConnectivityOnly) {
            $ProbeTimeoutMs = 3000
            $watch = [Diagnostics.Stopwatch]::StartNew()
            $status = Get-HttpStatusThroughProxy -Uri 'https://www.google.com/generate_204'
            $watch.Stop()
            if ($status -ne 204) { throw 'probe_latency_failed' }
            return @{ Connectivity=@{RttMs=[int]$watch.ElapsedMilliseconds}; Country='' }
        }
        $net = Test-GoogleConnectivity
        ...
        # The unique probe directory is retained for diagnostics; never delete user data.
    }
}
...
if ($null -eq $selectedCandidate) {
    Write-SafeLog -Event 'no_replacement_production_preserved' -Values @{port=$Port}
    Save-FailoverState -State $failoverState
