...
if (-not [bool]$policy.region_neutral_order) { throw 'region_order_must_be_country_neutral' }
if (-not [bool]$policy.account_scoped_state) { throw 'failover_state_must_be_account_scoped' }
if ([int]$policy.max_candidate_count -lt 48 -or [int]$policy.max_candidate_count -gt 128) { throw 'cross_subscription_pool_size_invalid' }
...
if ([string]$policy.location_failure_disposition -ne 'cooldown') { throw 'location_failure_must_use_cooldown' }
if (-not [bool]$policy.location_failure_preserves_history) { throw 'location_failure_must_preserve_success_history' }
if (-not [bool]$policy.failed_failover_restores_active_candidate) { throw 'failed_failover_must_restore_active_candidate' }
...
if (-not [bool]$policy.verified_history_beats_unverified_region) { throw 'verified_history_must_beat_unverified_region' }
...
Write-Output ('PASS failover policy candidates=' + [int]$policy.candidate_count +
    ...
    ' region_neutral=' + [bool]$policy.region_neutral_order +
